...
#This model is for patients receiving the new test who are identified as having early AKI and therefore can benefit from early intervention
...
model <- function(i,test) {                                   
  ...
  for (t in 1:T){               
    ...
    tps[5,1,t]   <-  0 
    tps[5,2,t]   <-  0                                #Base case: testing is conducted on patients not currently with known AKI.         
    tps[5,3,t]   <-  0                                         
    tps[5,4,t]   <-  0
    tps[5,5,t]   <-  1-tp_norm[i,"ward",t]-tp_norm[i,"ward_RRT",t]-tp_norm[i,"disch",t]-tp_norm[i,"disch_RRT",t]-tp_norm[i,"mort",t] - (tp_norm[i,"S1",t]*RR_early.treat[i])-(tp_norm[i,"S2",t]*RR_early.treat[i]) - (tp_norm[i,"S3",t]*RR_early.treat[i]) - (tp_norm[i,"RRT",t]*RR_early.treat[i])     
    tps[5,6,t]   <-  tp_norm[i,"S1",t]  *RR_early.treat[i]    #Treatment benefit - reduced rate of upward transitions through AKI states       
    tps[5,7,t]   <-  tp_norm[i,"S2",t]  *RR_early.treat[i]       
    tps[5,8,t]   <-  tp_norm[i,"S3",t]  *RR_early.treat[i]      
    tps[5,9,t]   <-  tp_norm[i,"RRT",t] *RR_early.treat[i]     
    ...
    tps[6,6,t]   <-  1-tp_S1[i,"S0",t]-tp_S1[i,"ward",t]-tp_S1[i,"ward_RRT",t]-tp_S1[i,"disch",t]-tp_S1[i,"disch_RRT",t]-tp_S1[i,"mort",t] - (tp_S1[i,"S2",t]*RR_early.treat[i]) - (tp_S1[i,"S3",t]*RR_early.treat[i]) - (tp_S1[i,"RRT",t]*RR_early.treat[i]) 
    tps[6,7,t]   <-  tp_S1[i,"S2",t]   *RR_early.treat[i]                    
    tps[6,8,t]   <-  tp_S1[i,"S3",t]   *RR_early.treat[i]       
    tps[6,9,t]   <-  tp_S1[i,"RRT",t]  *RR_early.treat[i]          
    ...
    tps[7,7,t]   <-  1-tp_S2[i,"S0",t]- tp_S2[i,"S1",t]-tp_S2[i,"ward",t]-tp_S2[i,"ward_RRT",t]-tp_S2[i,"disch",t]-tp_S2[i,"disch_RRT",t]-tp_S2[i,"mort",t] - (tp_S2[i,"S3",t]*RR_early.treat[i]) - (tp_S2[i,"RRT",t]*RR_early.treat[i]) 
    tps[7,8,t]   <-  tp_S2[i,"S3",t]   *RR_early.treat[i]        
    tps[7,9,t]   <-  tp_S2[i,"RRT",t]  *RR_early.treat[i]        
    ...
    tps[8,8,t]   <-  1-tp_S3[i,"S0",t]- tp_S3[i,"S1",t]- tp_S3[i,"S2",t]-tp_S3[i,"ward",t]-tp_S3[i,"ward_RRT",t]-tp_S3[i,"disch",t]-tp_S3[i,"disch_RRT",t]-tp_S3[i,"mort",t] - (tp_S3[i,"RRT",t]*RR_early.treat[i])
    tps[8,9,t]   <-  tp_S3[i,"RRT",t] *RR_early.treat[i]     
    ...
    tps[9,9,t]   <-  1-tp_RRT[i,"S0",t]- tp_RRT[i,"S1",t]- tp_RRT[i,"S2",t]- tp_RRT[i,"S3",t]-tp_RRT[i,"ward",t]-tp_RRT[i,"ward_RRT",t]-tp_RRT[i,"disch",t]-tp_RRT[i,"disch_RRT",t]- (1-exp(log(1-tp_RRT[i,"mort",t]) *RR_early.treat[i]))
    tps[9,10,t]  <-  tp_RRT[i,"ward",t] 
    tps[9,11,t]  <-  tp_RRT[i,"ward_RRT",t]               
    tps[9,12,t]  <-  tp_RRT[i,"disch",t] 
    tps[9,13,t]  <-  tp_RRT[i,"disch_RRT",t]         
    tps[9,14,t]  <-  1-exp(log(1-tp_RRT[i,"mort",t]) *RR_early.treat[i]) 
  ...
  trace [1,3,i]   <-  0
  trace [1,4,i]   <-  0
  trace [1,5,i]   <-  ifelse(test==1, start.S0.Nephro_TP[i], ifelse(test==21, start.S0.NGALp_TP[i], ifelse(test==22, start.S0.NGALu_TP[i],ifelse(test==23, start.S0.NGALs_TP[i],ifelse(test==31,start.S0.CystCp_TP[i],ifelse(test==32,start.S0.CystCu_TP[i],ifelse(test==33,start.S0.CystCs_TP[i],NA)))))))
  trace [1,6,i]   <-  ifelse(test==1, start.S1.Nephro_TP[i], ifelse(test==21, start.S1.NGALp_TP[i], ifelse(test==22, start.S1.NGALu_TP[i],ifelse(test==23, start.S1.NGALs_TP[i],ifelse(test==31,start.S1.CystCp_TP[i],ifelse(test==32,start.S1.CystCu_TP[i],ifelse(test==33,start.S1.CystCs_TP[i],NA)))))))
  trace [1,7,i]   <-  0  
  trace [1,8,i]   <-  0 
  trace [1,9,i]   <-  0
  trace [1,10,i]  <-  0  
