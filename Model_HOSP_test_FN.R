...
#This model is for False Negative patients- patients who are destined to get or have AKI but are indicated as negative on the new test
#These patients follow the baseline transitions for AKI i.e. they do not benefit from early intervention, and have the cost of the new test applied
...
#======================= TRANSITION PROBABILITY MATRIX ================================================================================================================
  ...
  for (t in 1:T){               
    ...
    tps[5,3,t]   <-  0                                         
    tps[5,4,t]   <-  0
    tps[5,5,t]   <-  tp_norm[i,"S0",t]     #Sensitivity Analysis 8 (SA8), change this to: 1- (tp_norm[i,"S1",t]+ tp_norm[i,"S2",t]+tp_norm[i,"S3",t]+ tp_norm[i,"RRT",t]  +tp_norm[i,"ward",t]  +tp_norm[i,"ward",t] +tp_norm[i,"disch",t]+tp_norm[i,"disch_RRT",t]+tp_norm[i,"mort",t]*1.1) #  #SA8: Increased mortality for early AKI False Negative results
    ...
    tps[5,14,t]  <-  tp_norm[i,"mort",t]*1.1      
    ...
    tps[6,3,t]   <-  0
    tps[6,4,t]   <-  0
    tps[6,5,t]   <-  tp_S1[i,"S0",t]      #SA8:  1- (tp_S1[i,"S1",t]+ tp_S1[i,"S2",t]+tp_S1[i,"S3",t]+ tp_S1[i,"RRT",t]  +tp_S1[i,"ward",t]  +tp_S1[i,"ward",t] +tp_S1[i,"disch",t]+tp_S1[i,"disch_RRT",t]+tp_S1[i,"mort",t]*1.1)  #                       
  ...
  trace [1,3,i]   <-  0
  trace [1,4,i]   <-  0
  trace [1,5,i]   <-  ifelse(test==1, start.S0.Nephro_FN[i], ifelse(test==21, start.S0.NGALp_FN[i], ifelse(test==22, start.S0.NGALu_FN[i], ifelse(test==23, start.S0.NGALs_FN[i], ifelse(test==31,start.S0.CystCp_FN[i],ifelse(test==32,start.S0.CystCu_FN[i],ifelse(test==33,start.S0.CystCs_FN[i],NA)))))))
  trace [1,6,i]   <-  ifelse(test==1, start.S1.Nephro_FN[i], ifelse(test==21, start.S1.NGALp_FN[i], ifelse(test==22, start.S1.NGALu_FN[i], ifelse(test==23, start.S1.NGALs_FN[i], ifelse(test==31,start.S1.CystCp_FN[i],ifelse(test==32,start.S1.CystCu_FN[i],ifelse(test==33,start.S1.CystCs_FN[i],NA)))))))
  trace [1,7,i]   <-  ifelse(test==1, start.S2.Nephro_FN[i], ifelse(test==21, start.S2.NGALp_FN[i], ifelse(test==22, start.S2.NGALu_FN[i], ifelse(test==23, start.S2.NGALs_FN[i], ifelse(test==31,start.S2.CystCp_FN[i],ifelse(test==32,start.S2.CystCu_FN[i],ifelse(test==33,start.S2.CystCs_FN[i],NA)))))))
  trace [1,8,i]   <-  ifelse(test==1, start.S3.Nephro_FN[i], ifelse(test==21, start.S3.NGALp_FN[i], ifelse(test==22, start.S3.NGALu_FN[i], ifelse(test==23, start.S3.NGALs_FN[i], ifelse(test==31,start.S3.CystCp_FN[i],ifelse(test==32,start.S3.CystCu_FN[i],ifelse(test==33,start.S3.CystCs_FN[i],NA)))))))
  trace [1,9,i]   <-  ifelse(test==1, start.RRT.Nephro_FN[i], ifelse(test==21, start.RRT.NGALp_FN[i],ifelse(test==22, start.RRT.NGALu_FN[i], ifelse(test==23, start.RRT.NGALs_FN[i], ifelse(test==31,start.RRT.CystCp_FN[i],ifelse(test==32,start.RRT.CystCu_FN[i],ifelse(test==33,start.RRT.CystCs_FN[i],NA)))))))
  trace [1,10,i]  <-  0  
  trace [1,11,i]  <-  0    
  trace [1,12,i]  <-  0 
  trace [1,13,i]  <-  0
  ...
  # Store trace
  trace<<-trace      #doesn't this need to have 10,000 simulations?? 
...
#======================= Outputs ============================================================================================================================
  ...
  cost <- sum(cost[1:T]) + #Health state costs
    (trace [1,5,i] + trace [1,6,i]+ trace [1,7,i]+ trace [1,8,i]+ trace [1,9,i])*
    (ifelse(test==1, c_Nephro[i], ifelse(test==21 | test==22 |test==23, c_NGAL[i], ifelse(test==31 | test==32 | test==33,c_CystC[i], 0)))) #+   #SA8:Cost saving from negative result  #Cost of doing test
    #(trace [1,5,i] + trace [1,6,i])*(-200)  #SA8: cost saving for early AKI negative test results. 
