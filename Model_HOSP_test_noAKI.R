...
#This model is for patients receiving the new test who do not have AKI (includes True Negatives and False Positives)
#Nb: False Positive proportion have cost of early AKI intervention applied to the costs at the end of this code
...
#======================= TRANSITION PROBABILITY MATRIX ================================================================================================================
  ...
  for (t in 1:T){               
    ...
    tps[1,1,t]   <-  1- (tp_ICUtoward[i,t] + tp_ICUtodisch[i,t] + tp_ICUmort[i,t])#*ifelse(test==1,((start.noAKI.Nephro_TN[i]/start.noAKI.Nephro[i]) +(1.1*(start.noAKI.Nephro_FP[i]/start.noAKI.Nephro[i]))), 
                                                                                  #ifelse(test==21,((start.noAKI.NGALp_TN[i]/start.noAKI.NGALp[i]) +(1.1*(start.noAKI.NGALp_FP[i]/start.noAKI.NGALp[i]))),
                                                                                  #ifelse(test==33,((start.noAKI.CystCs_TN[i]/start.noAKI.CystCs[i]) +(1.1*(start.noAKI.CystCs_FP[i]/start.noAKI.CystCs[i]))),NA)))))      #SA7 & 8:mortality impact for FP patients
    tps[1,2,t]   <-  tp_ICUtoward[i,t]                       
    tps[1,3,t]   <-  tp_ICUtodisch[i,t]             
    tps[1,4,t]   <-  tp_ICUmort[i,t]#*ifelse(test==1,((start.noAKI.Nephro_TN[i]/start.noAKI.Nephro[i]) +(1.1*(start.noAKI.Nephro_FP[i]/start.noAKI.Nephro[i]))), 
                                     #ifelse(test==21,((start.noAKI.NGALp_TN[i]/start.noAKI.NGALp[i]) +(1.1*(start.noAKI.NGALp_FP[i]/start.noAKI.NGALp[i]))),
                                    #ifelse(test==33,((start.noAKI.CystCs_TN[i]/start.noAKI.CystCs[i]) +(1.1*(start.noAKI.CystCs_FP[i]/start.noAKI.CystCs[i]))),NA)))      #SA7 & 8:mortality impact for FP patients
    tps[1,5,t]   <-  0
    tps[1,6,t]   <-  0                                                 
    ...
    tps[2,1,t]   <-  0
    tps[2,2,t]   <-  1 - (tp_wardtodisch[i]  + tp_wardmort[i])                                     
    tps[2,3,t]   <-  tp_wardtodisch[i]                                                        
    tps[2,4,t]   <-  tp_wardmort[i]                                                                    
    tps[2,5,t]   <-  0
    tps[2,6,t]   <-  0                             
    tps[2,7,t]   <-  0
    tps[2,8,t]   <-  0
    ...
    tps[3,1,t]   <-  0
    tps[3,2,t]   <-  0
    tps[3,3,t]   <-  1- tp_dischmort[i]        
    tps[3,4,t]   <-  tp_dischmort[i]
    tps[3,5,t]   <-  0
    tps[3,6,t]   <-  0                        
    tps[3,7,t]   <-  0
    tps[3,8,t]   <-  0
  ...
  trace [1,1,i]   <-  ifelse(test==1, start.noAKI.Nephro[i], ifelse(test==21, start.noAKI.NGALp[i], ifelse(test==22, start.noAKI.NGALu[i],ifelse(test==23, start.noAKI.NGALs[i],ifelse(test==31,start.noAKI.CystCp[i],ifelse(test==32,start.noAKI.CystCu[i],ifelse(test==33,start.noAKI.CystCs[i],NA)))))))
  trace [1,2,i]   <-  0
  trace [1,3,i]   <-  0
  trace [1,4,i]   <-  0
  trace [1,5,i]   <-  0
  ...
  cost <- sum(cost[1:T])+
    (c_early.treat[i]*ifelse(test==1,start.noAKI.Nephro_FP[i], ifelse(test==21, start.noAKI.NGALp_FP[i], ifelse(test==22, start.noAKI.NGALu_FP[i], ifelse(test==23, start.noAKI.NGALs_FP[i], ifelse(test==31, start.noAKI.CystCp_FP[i], ifelse(test==32, start.noAKI.CystCu_FP[i], ifelse(test==33, start.noAKI.CystCs_FP[i], NA)))))))) +  #Cost of early intervention for FP's 
    ifelse(test==1, c_Nephro[i], ifelse(test==21 | test==22 | test==23, c_NGAL[i], ifelse(test==31 | test==32 | test==33,c_CystC[i], NA)))*(trace[1,1,i]) #+ 
  # (-200)*(ifelse(test==1,start.noAKI.Nephro_TN[i], ifelse(test==21, start.noAKI.NGALp_TN[i],ifelse(test==33,start.noAKI.CystCs_TN[i],NA))))    #SA8: cost saving for negative result
