...
#This model is for the standard care baseline arm i.e. none of the new tests are implemented in this model
...
#                             5=  In ICU with current normal kidney function (AKI cohort) 
...
# Create empty objects to be filled
...
#======================= TRANSITION PROBABILITY MATRIX ================================================================================================================
  ...
  for (t in 1:T){               
    ...
    #Transitions from ICU with normal kidney function (no AKI cohort) [transitions from state 1, dependant on time]     
    tps[1,1,t]   <-  1- (tp_ICUtoward[i,t] + tp_ICUtodisch[i,t] + tp_ICUmort[i,t])    #i.e. probaility of remaining in state #1                                                              
    tps[1,2,t]   <-  tp_ICUtoward[i,t]        #Probabiity of moving to general hospital ward               
    tps[1,3,t]   <-  tp_ICUtodisch[i,t]       #Probability of being discharged
    tps[1,4,t]   <-  tp_ICUmort[i,t]          #Probability of dying
    tps[1,5,t]   <-  0
    tps[1,6,t]   <-  0                                                 
    tps[1,7,t]   <-  0     
    tps[1,8,t]   <-  0    
  ...
  trace [1,1,i]   <-  start.noAKI[i]
  trace [1,2,i]   <-  0
  trace [1,3,i]   <-  0
  trace [1,4,i]   <-  0
  trace [1,5,i]   <-  start.S0[i]
  trace [1,6,i]   <-  start.S1[i]
  trace [1,7,i]   <-  start.S2[i]   
  trace [1,8,i]   <-  start.S3[i] 
  trace [1,9,i]   <-  start.RRT[i]
  trace [1,10,i]  <-  0  
  trace [1,11,i]  <-  0    
  trace [1,12,i]  <-  0 
  trace [1,13,i]  <-  0
